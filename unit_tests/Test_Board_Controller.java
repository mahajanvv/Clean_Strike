...
public class Test_Board_Controller {
    private static boolean Test_Increase_Coins_Count(Board_Controller board){
        // Test-1 Increasing Black coins count by 10
            // Before value = 0
        board.Increase_Coins(Coin_Type.BLACK, 10);
        if(board.get_coins_count(Coin_Type.BLACK) != 10){
        ...
        // Test-2 Increasing Red Coins count by 2
            // Before value = 0
        board.Increase_Coins(Coin_Type.RED, 2);
        if(board.get_coins_count(Coin_Type.RED) != 2){
    ...
    private static boolean Test_Decrease_Coins_Count(Board_Controller board){
        // Test-1 Decreasing Black coins count by 2
            // Before value = 9
        board.Decrease_Coins(Coin_Type.BLACK, 2);
        if(board.get_coins_count(Coin_Type.BLACK) != 7){
        ...
        // Test-2 Decreasing Red Coins Count by 1
            // Before value = 1
        board.Decrease_Coins(Coin_Type.RED, 1);
        if(board.get_coins_count(Coin_Type.RED) != 0){
    ...
    public static void main(String []args){
        ...
        boolean test1 = Test_Increase_Coins_Count(board);
        if(test1){
        ...
        boolean test2 = Test_Decrease_Coins_Count(board);
        if(test2){
